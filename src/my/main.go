...
func (s *XYStack) Push(value []int) {
	s.storage = append(s.storage, value)
}

func (s *XYStack) Pop() ([]int, error) {
	last := len(s.storage) - 1
	if last <= -1 {
		return []int{0}, errors.New("stack is empty")
	}
...
func ifPathExists(layout [][]int, start []int, finish []int) bool {
	...
	for {
		...
		if y-1 >= 0 && layout[y-1][x] != 8 {
			// we're moving up
			solver.Push([]int{x, y - 1})
		}
		// if the right path is clear x+1
		if x+1 < len(layout)-1 && layout[y][x+1] != 8 {
			solver.Push([]int{x + 1, y})
		}

		// if the left path is clear x-1
		if x-1 >= 0 && layout[y][x-1] != 8 {
			solver.Push([]int{x - 1, y})
		}

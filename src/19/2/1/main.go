...
func demultiplexingFunc(dataSourceChan chan int, amount int) ([]chan int, <-chan int) {
	...
	// представлять так называемое нулевое значение для данного типа.
	// Так как тип у нас канала - ссылочный тип - то все элементы будут равны nil
	for i := range output {
...
func multiplexingFunc(done <-chan int, channels ...chan int) <-chan int {
	...
	// Именно его мы и вернем из этой функции для употребления внешним кодом
	multiplexedChan := make(chan int)
	multiplex := func(c <-chan int) {
		defer wg.Done()
		for {
			select {
			// Если поступило сообщение из одного из каналов-источников перенаправляем его в общий канал
			case i := <-c:
				multiplexedChan <- i
			case <-done:
...
func main() {
	...
	// Функция создает свой собственный канал и посылает в него пять сообщений
